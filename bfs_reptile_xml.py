...
def c_source(url, i):
    ...
    items = soup.find_all("code", attrs={"class": re.compile(r"language-XML|language-xml|language-cobol")})
    
    for item in items:
        c_source = item.text.strip()
        with open("data/libxml/format_set/org_xml:id:{}".format(str(i).zfill(4)), "w", encoding='utf-8') as f:
            f.write(c_source)
        f.close()
        i += 1
    return i
...
if __name__ == '__main__':
    ...
    url = "https://blog.csdn.net/weixin_50991263/article/details/124287873"
    BFS(url, i)
